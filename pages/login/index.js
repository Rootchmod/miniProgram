    ...
    if (this.data.verificationCode === this.data.verificationCodeInput) {
      ...
      try {
        ...
        if (res1.statusCode === 200 && res1.data) {
          app.globalData.userId = res1.data[0].F_Id;
          console.log(JSON.stringify(res1.data[0].F_Id)+"=====================")
          console.log(app.globalData.userId + "000000");

          //RootChmod 根据UserId获取用户信息
          const userInfo = app.request()
            .get(`${config.requestUrl}User/getUserByUserId`)
            .query({
              userId: res1.data[0].F_Id
            })
            .end();

          userInfo.then(res => {
            console.log('公司查询' + JSON.stringify(res));
            res = res.data[0];
            wx.setStorageSync("CompanyId", res.CompanyId);
            wx.setStorageSync("userId", res.F_Id);
            wx.setStorageSync("user_Id", res.F_Id);
